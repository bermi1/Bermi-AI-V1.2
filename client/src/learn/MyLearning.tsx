...
// A course, program, and resource are each a different kind of relationship
// to an organization's offering — this page groups by kind so "3/5 lessons
// done" (a course) doesn't sit next to "Available" (a resource) as if they
  ...
  return (
    <div className="mx-auto max-w-4xl px-4 py-6 md:py-10">
      ...
      <p className="mb-6 text-[14px] text-ink-muted">
        Everything you've enrolled in or been given access to, and your progress.
      </p>
      ...
      {enrollments.length === 0 ? (
        <EmptyState
          ...
          body="Browse the catalog and enroll or get access to something to see it here."
        />
      ) : (
                      ...
                      <div key={e.id} className="flex flex-col rounded-2xl border border-edge bg-surface-raised p-5">
                        <div className="mb-3 flex items-start gap-3">
                          <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-primary-soft text-primary">
                            <CourseIcon name={course.cover_emoji} size={20} />
                          </div>
                        ...
                        <p className="mb-4 text-[12.5px] text-ink-faint">
                          {kind === 'resource' ? 'Ready whenever you are' : `${doneCount} ${meta.doneWord}${doneCount === 1 ? '' : 's'} completed`}
                        </p>
